#include <stdbigos/buffer.h>

int buffer_read_u32_be(const buffer_t* buf, size_t offset, u32* out) {
	if (!buf || !out || offset + 4 > buf->size)
		return BUFFER_OUT_OF_BOUNDS;
	const u8* b = (const u8*)buf->data + offset;
	*out = ((u32)b[0] << 24) | ((u32)b[1] << 16) | ((u32)b[2] << 8) | (u32)b[3];
	return BUFFER_OK;
}

int buffer_read_u64_be(const buffer_t* buf, size_t offset, u64* out) {
	if (!buf || !out || offset + 8 > buf->size)
		return BUFFER_OUT_OF_BOUNDS;
	const u8* b = (const u8*)buf->data + offset;
	*out = ((u64)b[0] << 56) | ((u64)b[1] << 48) | ((u64)b[2] << 40) | ((u64)b[3] << 32) | ((u64)b[4] << 24) |
	       ((u64)b[5] << 16) | ((u64)b[6] << 8) | (u64)b[7];
	return BUFFER_OK;
}

int buffer_read_u32_le(const buffer_t* buf, size_t offset, u32* out) {
	if (!buf || !out || offset + 4 > buf->size)
		return BUFFER_OUT_OF_BOUNDS;
	const u8* b = (const u8*)buf->data + offset;
	*out = ((u32)b[3] << 24) | ((u32)b[2] << 16) | ((u32)b[1] << 8) | (u32)b[0];
	return BUFFER_OK;
}

int buffer_read_u64_le(const buffer_t* buf, size_t offset, u64* out) {
	if (!buf || !out || offset + 8 > buf->size)
		return BUFFER_OUT_OF_BOUNDS;
	const u8* b = (const u8*)buf->data + offset;
	*out = ((u64)b[7] << 56) | ((u64)b[6] << 48) | ((u64)b[5] << 40) | ((u64)b[4] << 32) | ((u64)b[3] << 24) |
	       ((u64)b[2] << 16) | ((u64)b[1] << 8) | (u64)b[0];
	return BUFFER_OK;
}

int buffer_read_cstring(const buffer_t* buf, size_t offset, const char** out_str) {
	if (!buf || !buf->data || !out_str || offset >= buf->size)
		return BUFFER_OUT_OF_BOUNDS;

	const char* base = (const char*)buf->data;
	size_t i = offset;
	// scan until '\0' or end‐of‐buffer
	while (i < buf->size && base[i] != '\0') {
		i++;
	}
	if (i >= buf->size)
		return BUFFER_OUT_OF_BOUNDS; // no terminator inside buffer

	*out_str = base + offset;
	return BUFFER_OK;
}
